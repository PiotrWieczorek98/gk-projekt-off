...
public class MovementControllerLowPolyFPSPack : MonoBehaviour {
	...
	private void Update () 
	{
        ...
        {
            transform.Translate(Vector3.up * jumpStrength * 0.01f);
        }
        //transform.Translate(Vector3.up * Physics.gravity.y * Time.deltaTime);
		...
		transform.eulerAngles = new Vector2(0, rotation.y) * mouseLookSpeed;
		...
		arms.transform.localRotation = Quaternion.Euler (rotation.x * mouseLookSpeed, 0, 0);
	}
}